...
<div class="container">
    ...
    <div class="row">
        <div class="col-8 offset-2 table-responsive mt-3">
            <form action="{{route('hobbies.index')}}" method="GET">
                <div class="row">
                    <div class="col-6">
                    ...
                    <div class="col-3">
                        <a href="{{route('hobbies.create')}}" class="btn btn-success w-100">+ Dodaj novi hobi</a>
                    </div>
                </div>
            </form>
            ...
            <table class="table table-hover">
                ...
                @foreach($hobbies as $hobby)
                    <tr>
                        <td>{{$hobby->id}}</td>
                        <td>{{$hobby->name}}</td>
                        <td><a href="{{route('hobbies.edit',['id'=>$hobby->id])}}" class="btn btn-primary btn-sm">Izmjena</a></td>
                        <td>
                            <form action="{{route('hobbies.delete',['id'=>$hobby->id])}}" method="post">
                                @method('DELETE')
                                @csrf
